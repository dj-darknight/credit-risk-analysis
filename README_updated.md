...
Extends the above pipeline with three diagnostics, run on the same data,
...
- **Learning curves** (train vs. validation ROC-AUC across 6 training-set
  sizes).
- **Fairlearn bias audit** on the HCDR Decision Tree, using applicant age
  band as the sensitive attribute (demographic parity, equalized odds).

**Note on kNN sample size:** cross-validation and learning curves for kNN
use an 8,000-row stratified subsample of each dataset, not the full
150k/215k rows. Distance-weighted kNN CV/learning-curve computation on the
full datasets was not feasible in standard compute environments. Decision
Tree steps use the full dataset in both cases. This is disclosed here and
in the assignment report rather than applied silently.

Saves `part2_results.json` with all CV, learning-curve, and fairness
metrics used in the Part 2 write-up. Expected runtime: under a minute.
